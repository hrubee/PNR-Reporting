...
export default function SymphonyEquipmentForm({
  today,
  todayLabel,
  ...
  const categories = Array.from(new Set(SYMPHONY_EQUIPMENT_ITEMS.map((i) => i.category)));
  ...
  function startEditSubmission(entry: EntryType) {
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        const normalized = SYMPHONY_EQUIPMENT_ITEMS.map((itemDef) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/entries/symphony-equipment", {
        method: "POST",
      ...
      if (editingId) {
        ...
        setAlert({ type: "success", msg: "✅ Symphony World equipment log updated successfully!" });
      } else {
        setTodayEntries((prev) => [savedFormatted, ...prev]);
        setHistory((prev) => [savedFormatted, ...prev]);
        setAlert({ type: "success", msg: "✅ Symphony World equipment checklist submitted successfully!" });
      }
    ...
    <div className="page-container fade-in">
      ...
      <div className="page-header">
        <div className="page-header-text">
          <h1>🎼 Symphony World Equipment & Hygiene Checklist</h1>
          <p>{todayLabel} · Daily Equipment Cleaning & Sanitation Checklist (32 Items)</p>
        </div>
          ...
          <div className="card">
            <div className="card-header">
              <div className="card-title">
                {editingId ? "✏️ Edit Symphony World Equipment Log" : "📝 New Symphony World Equipment Record"}
              </div>
            </div>
      ...
      <div className="card">
        <div className="card-header">
          <div className="card-title">🕒 Symphony World Equipment History Log</div>
